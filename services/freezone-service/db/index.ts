...
const getConnectionUrl = (): string => {
  const connectionUrl = process.env.FREEZONE_SERVICE_DB_URL || process.env.DATABASE_URL;
  
  if (!connectionUrl) {
    throw new DatabaseException(
      'Database connection URL not found in environment variables. ' +
      'Set FREEZONE_SERVICE_DB_URL or DATABASE_URL.'
    );
  }
...
const checkConnection = async (): Promise<void> => {
  try {
    ...
    console.log('[FreezoneService] Database connection established successfully');
  } catch (error) {
    console.error('[FreezoneService] Database connection failed:', error);
...
process.on('SIGINT', async () => {
  try {
    await pool.end();
    console.log('[FreezoneService] Database connection pool closed successfully');
    process.exit(0);
  } catch (error) {
    console.error('[FreezoneService] Error closing database connection:', error);
    process.exit(1);
  }
});
