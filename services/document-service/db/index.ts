...
const getConnectionUrl = (): string => {
  const connectionUrl = process.env.DOCUMENT_SERVICE_DB_URL || process.env.DATABASE_URL;
  
  if (!connectionUrl) {
    throw new DatabaseException(
      'Database connection URL not found in environment variables. ' +
      'Set DOCUMENT_SERVICE_DB_URL or DATABASE_URL.'
    );
  }
...
const checkConnection = async (): Promise<void> => {
  try {
    ...
    console.log('[DocumentService] Database connection established successfully');
  } catch (error) {
    console.error('[DocumentService] Database connection failed:', error);
...
process.on('SIGINT', async () => {
  try {
    await pool.end();
    console.log('[DocumentService] Database connection pool closed successfully');
    process.exit(0);
  } catch (error) {
    console.error('[DocumentService] Error closing database connection:', error);
    process.exit(1);
  }
});
